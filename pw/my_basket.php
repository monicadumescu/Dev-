		  ...
		  <?php
				...
				if ($conn->connect_error) {
                 die("Connection failed: " . $conn->connect_error);
				   ...
				   $sql_out="select products from BasketTable where email=$email";
				   ...
				   function delete_data($id)
				   {
				       ...
				       $conn = new mysqli($host, $user, $pass,$database);
					   $sql_del="delete from BasketTable where email=$email";
					   $result_del= mysqli_query($conn,$sql_del);
                       if($result_del){
                        echo "Product deleted from basket successessfuly";
                        }else{
                          echo "Error";
                        }
				   }
				  if(mysqli_num_rows($result_out))
				  {
					  ...
					  {
							  echo  "<div class='div2'><style> .div2 { border-style:groove;}</style>" ,$row["products"], 
						'</br><button class="btn btn-outline-success my-1 my-sm-1"  >Delete</button></div></br>';
