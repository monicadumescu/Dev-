				...
				if ($conn->connect_error) {
                 die("Connection failed: " . $conn->connect_error);
				   ...
				   function delete_data($id)
				   {
				       ...
				       $conn = new mysqli($host, $user, $pass,$database);
					   $sql_del="delete from CommentsTable where id='$id' ";
					   $result_del= mysqli_query($conn,$sql_del);
                       if($result_del){
                         $mes ="Comment deleted successessfuly";
                        }else{
                          $mes="Error";
                        }
						return $mes;
				  ...
				  if($count >0)
				  {
					  while($row=$result_out->fetch_assoc())
					  {
						...
						</style> ", $row["email"],"</div>", "<div class='div2'><style> .div2 { border-style:groove;}</style>" ,$row["comments"], 
						'</br><button class="btn btn-outline-success my-2 my-sm-1" onclick="'.delete_data($row['id']).'" type="submit">Delete</button></div></br>';
